function Results = vs_solver(n,LidVel,Re,dt,tmax,tol,SOR,MG)
...
for t = dt:dt:tmax
    ...
    vorticity(:,1) = - 2 * stream(1:nx, 2) / (dx^2);
    vorticity(1,:) = -2 * stream(2, 1:nx) / (dx^2);
    vorticity(nx,:) = -2 * stream(nx - 1, 1:nx) / (dx^2);
